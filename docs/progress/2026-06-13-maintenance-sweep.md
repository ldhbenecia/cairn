...
> 진행 단계: 유지보수 — 전체 점검 + 성능 + 프롬프트 개선 (마무리)
> 상태: 완료
...
- feat(github): 수집·요약을 내 개발 작업 중심으로 — 리뷰/코멘트 검색 제거(계정당 3→1), assigned 카테고리 추가, PR body·커밋 기반 상세 요약 (PR #91)
- chore(release): 0.17.7 → 0.19.0 두 단계 bump 를 0.18.0 한 번으로 정정 (v0.19.0 태그·드래프트 릴리스 삭제)
- refactor: 프롬프트를 daily-prompt.ts/rollup-prompt.ts 로 분리, 환경설정 다이얼로그 774줄 → 셸 + 탭별 9개 파일 (PR #92)
- v0.18.0 태그 + GitHub Release
...
- 버전: 한 작업 묶음 = minor 한 번. PR 마다 feat 라고 minor 를 거듭 올리지 않기 (사용자 피드백)
- 리뷰 활동은 일지 가치가 낮다는 피드백 — 수집 자체를 끊는 쪽이 속도·노이즈 모두 이득. 과거 페이지 호환 위해 reviewedBullets 필드는 `default([])` 로 유지
- 발행 속도 실측 (단일 일자, --force): collect 2.6s / **summarize 123.3s** / publish 2.1s — 요약(Claude CLI 세션)이 wall-clock 의 96%. 수집·발행은 더 줄일 것이 거의 없음
- 남은 속도 레버: 요약 모델 변경(haiku 등 — 상세함과 트레이드오프라 보류), 백필 동시성 2 유지(동시 Claude 프로세스 메모리 고려)

## 다음

- (없음 — sweep 종료. 후속 아이디어: 요약 모델 선택 옵션, 리퀴드 글래스 패키징 blur 재조사)
