...
const FONT_DIR = 'app/_fonts';

export default async function OpengraphImage() {
  const [sans, sansSemibold, sansBold] = await Promise.all([
    readFile(join(process.cwd(), FONT_DIR, 'Inter-400.woff')),
    readFile(join(process.cwd(), FONT_DIR, 'Inter-600.woff')),
    readFile(join(process.cwd(), FONT_DIR, 'Inter-700.woff')),
  ]);
  return new ImageResponse(
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
        <svg width={38} height={52} viewBox="355 233 558 766" fill={ACCENT}>
          <g transform="translate(0,1254) scale(0.1,-0.1)">
            <path d={BRAND_PATH} />
          </g>
        </svg>
        <div style={{ fontSize: 38, fontWeight: 600, letterSpacing: -0.5 }}>cairn</div>
      </div>
    ...
    </div>,
    {
      ...
      fonts: [
        ...
        { name: 'Inter', data: sansSemibold, style: 'normal', weight: 600 },
