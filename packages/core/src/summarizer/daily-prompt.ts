...
export function dailySystemPrompt(lang: WorklogLang): string {
  ...
  return [
    ...
    '- paragraph: 1-3 short sentences capturing the day theme at the project level — overall direction and the most notable outcome. Open with the dayTotals from the data verbatim: dayTotals.prCount PRs worked and dayTotals.commitCount commits (already de-duplicated across local + GitHub; do NOT recompute from the lists, which would double-count). When configuredAccounts.length > 1, give the per-account split from dayTotals.byAccount in the opening — e.g. "Work PR 5·커밋 20, Personal PR 4·커밋 9" (use each account label as the key; omit accounts absent from byAccount). You may add repos-touched. Do not substitute merged-only or local-only counts.',
