...
function systemPrompt(lang: WorklogLang): string {
  ...
  return [
    ...
    '- doneBullets: 3-10 phrases, format "[project] meaningful work unit — outcome". project = repo name. Use only authored work from done.prs and done.commits. Group related commits/PRs of the same repo into one bullet. Emphasize OUTCOMES and impact (what shipped, what improved, performance/metrics, before→after, scope) over listing commits. Mine PR.body for the "why" and results; use commitsOnDate keywords for specifics. Avoid over-abstraction and avoid mere commit enumeration.',
    '- reviewedBullets: reviewed/commented PRs from reviewed.prs only. Phrase as review/approval/support work, never as implementation by the user.',
