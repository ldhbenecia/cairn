...
cairn 은 매일의 개발 활동 — 여러 레포에 걸친 GitHub PR 과 로컬 Git 커밋 — 을 Claude 로 요약해 로컬 마크다운 일지로 기록하고, 주간·월간 롤업까지 자동으로 만들어 줍니다. Notion 같은 연동을 켜면 그쪽으로도 발행됩니다.
...
- **연동** — 선택적으로 Notion 페이지 발행, 임의의 마크다운 폴더(예: Obsidian vault)로 미러
- **데스크톱 앱** — 첫 실행 가이드 설정, 원클릭 발행, 원하는 시각의 opt-in 자동 발행(기기 로컬 타임존), 인앱 일지 뷰어, 통계 대시보드, 일지 연결망 그래프, 다크/라이트/시스템 테마, 한/영
...
- GitHub fine-grained PAT (읽기 전용) 그리고/또는 수집할 로컬 Git 레포

선택: Notion 에도 발행하고 싶다면 Notion internal integration 토큰.
